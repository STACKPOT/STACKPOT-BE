...
public class UserCommandServiceImpl implements UserCommandService{
    ...
    public String createNickname(Role role) {
        ...
        while (true) {
            // 닉네임 생성
            String prompt = "닉네임을 생성하려고 해. 닉네임은 '양파', '버섯', '브로콜리', '당근' 앞에 어울리는 수식어를 붙여 만들 거야. " +
                    "수식어는 15글자 미만이어야 하고, 이전에 생성된 수식어는 반복하지 말아야 해. " +
                    "단순한 형용사뿐만 아니라 독창적인 표현(예: 비유적 표현, 사물이나 행동에서 착안한 표현)도 가능해. " +
                    "응답에는 앞에 숫자를 붙이지 말고, '양파', '버섯', '브로콜리', '당근' 같은 단어는 포함하지 말아줘." +
                    "단, 수식어는 뒤에 '양파', '버섯', '브로콜리', '당근'을 붙였을 때 말이 되어야해"+
                    "마지막으로 하나의 단어만 만들어줘  ";
