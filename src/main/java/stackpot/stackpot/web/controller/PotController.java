...
"""
    ...
    // 특정 팟의 상세정보 조회
    @Operation(summary = "팟 상세 조회 API (타인의 팟 상세보기-지원하기 버튼)", description = "'타인의 팟 상세보기-지원하기 버튼' 페이지에서 사용되는 정보를 리턴합니다.\n모든 팟에서 특정 팟을 선택하면 리턴하는 정보입니다.\npotId를 통해 특정 팟에 대한 상세정보를 조회할 수 있습니다.\n 모든 팟에서 특정 팟을 선택했을 때 보이는 상세 정보를 리턴합니다.")
    @GetMapping("/{pot_id}")
    ...
    // 특정 팟 지원자의 좋아요 상태 변경
    @Operation(summary = "특정 팟 지원자의 '마음에 들어요' 상태 변경 API (<내가 만든 팟>에서 상세보기 - 지원자 있음)", description = "'<내가 만든 팟>에서 상세보기 - 지원자 있음'에서 지원자의 좋아요 상태를 리턴합니다.\n지원자의 아이디와 liked 값을 true, false (Boolean)로 request 해 주시면 지원자의 liked 상태값이 해당 값에 맞춰 변경됩니다.")
    ...
    @Operation(summary = "특정 팟의 '마음에 들어요' 지원자 조회 API (<내가 만든 팟>에서 상세보기 - 지원자 있음)", description = "'<내가 만든 팟>에서 상세보기 - 지원자 있음'에 필요한 지원자 정보를 리턴합니다.\n지원자의 id, pot 지원 역할, 지원 역할에 따른 팟에서의 nickname, like 상태 값을 반환합니다. ")
    ...
    // 사용자가 지원한 팟 조회
    @Operation(summary = "지원한 팟 조회 API (지원한 팟 조회)", description = "'지원한 팟 조회'에 필요한 본인이 지원한 팟들 리스트를 리턴합니다.")
    @GetMapping("/apply")
    ...
    // Pot 내용 AI 요약
    @Operation(summary = "팟 구인글 AI 요약 API (팟을 다 끓였어요 클릭 시 작성 페이지)", description = "'팟을 다 끓였어요 클릭 시 작성 페이지'에서 AI 요약 생성 버튼을 누를시 생성되는 potSummary 내용입니다.\n팟의 구인글 내용을 활용해 작성되며, 해당 내용은 '팟 다 끓이기'를 할 때 potContent(구인글 내용)가 아닌 potSummary(소개글 내용)에 넣어주시면 됩니다.")
    ...
    @Operation(summary = "사용자별 끓인 팟 조회 API(마이페이지 - 끓인 팟)",
    ...
    @Operation(summary = "팟 다 끓이기 API (소개 작성 후 완료 시 모달)",
            description =
